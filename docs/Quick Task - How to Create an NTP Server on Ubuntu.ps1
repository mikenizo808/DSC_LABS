# How to Create an NTP Server on Ubuntu

## Introduction
<#

    This is an optional portion of the lab setup.
    For home lab, using public NTP instead of this setup would be fine.
    This is good practice though.

    In this demo we install chrony on Ubuntu to
    create an NTP Server for our lab.local domain.
    We modify the default time servers and add our own.

    Note: This was tested on Ubuntu 18.x, but should be fine on modern versions as well.
#>
...
#Note: You cannot use 'telnet' to test port 123 against Ubuntu.
#You must use a real ntp client to query your new NTP server.

## Command to test ntp from a working host

    watch ntpq -p localhost

## Summary
## This demo showed how to create an NTP server on Ubuntu.
